...
mkdir -p "${LOGS}"
echo "================================================================"
echo "[$(date +%H:%M:%S)] Starting rl_lora_json"
echo "================================================================"
UV_FROZEN=1 PATH="$(pwd)/.venv/bin:$PATH" .venv/bin/rl @ \
    ../environments/env_2048_text/configs/rl_lora_json.toml \
    > "${LOGS}/rl_lora_json.log" 2>&1
echo "[$(date +%H:%M:%S)] Finished rl_lora_json with exit code $?"
