...
mkdir -p "${LOGS}"
echo "================================================================"
echo "[$(date +%H:%M:%S)] Starting rl_lora_thinkprp"
echo "================================================================"
UV_FROZEN=1 PATH="$(pwd)/.venv/bin:$PATH" .venv/bin/rl @ \
    ../environments/env_2048_text/configs/rl_lora_thinkprp.toml \
    > "${LOGS}/rl_lora_thinkprp.log" 2>&1
echo "[$(date +%H:%M:%S)] Finished rl_lora_thinkprp with exit code $?"
